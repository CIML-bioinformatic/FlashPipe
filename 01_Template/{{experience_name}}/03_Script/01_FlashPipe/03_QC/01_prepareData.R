...
cat("<BR><b>Filtered plates to analyse:</b>", paste( "<BR>&nbsp;&nbsp;&nbsp;&nbsp;", filtered_origin_plate_name_list, collapse=""))
...
if (length(plate_errors_cell_barcode_well) > 0) {
  cat(" <BR>")
  cat("\n••••• ERROR DETECTED IN CELL BARCODE FILE (01_reference) FROM SCRIPT '01_prepareData' •••••\n")
...
if (length(plate_errors_zUMIs_output) > 0) {
  cat(" <BR>")
  cat("\n••••• ERROR DETECTED IN zUMIs OUTPUT FROM SCRIPT '01_prepareData' •••••\n")
...
if (length(plate_errors_ERCC) > 0) {
  cat(" <BR>")
  cat("\n••••• ERROR DETECTED IN ERCC FILE FROM SCRIPT '01_prepareData' •••••\n")
...
if (PARAMS_INDEXSORT == TRUE){
    ...
    if (file.info(path_index_sort_file)$size > 1) {
      ...
      for (column_name_in_not_fluorescent in NOT_FLUORESCENT){
        if (!column_name_in_not_fluorescent %in% colnames(index_sort_df)){
          plate_errors_indexsort <- c(plate_errors_indexsort, paste("<BR>\nERROR : There is no column named ", column_name_in_not_fluorescent," (the name here is the one provided in the config user) in the indexsort file for plate : ", plate_name))
        }
      }
  ...
  if (length(plate_errors_indexsort) > 0) {
    cat(" <BR>")
    cat("\n••••• ERROR DETECTED IN INDEXSORT FILE FROM SCRIPT '01_prepareData' •••••\n")
...
if (PARAMS_METADATA){
  ...
  if (length(plate_errors_metadata) > 0) {
    cat(" <BR>")
    cat("\n••••• ERROR DETECTED IN METADATA FILE (gsf) FROM SCRIPT '01_prepareData' •••••\n")
    cat(plate_errors_metadata, "\n")
    stop()
  }
  
  # Environment cleanup: removes temporary objects
  if( exists( "sheets_gsf")) rm("sheets_gsf")
  if( exists( "plate_sheet_verify")) rm("plate_sheet_verify")
  if( exists( "plate_sheet")) rm("plate_sheet")
  if( exists( "plate_name_verification")) rm("plate_name_verification") 
}
...
if (PARAMS_BCR | PARAMS_TCR){
  ...
  for (plate_name in levels(PLATES_LIST)) {
    ...
    if (!COLUMN_HEADER_C_CALL %in% colnames(file_air_tsv)){
      plate_errors_trust4 <- c(plate_errors_trust4, paste0("<BR>\nERROR : C_CALL (", COLUMN_HEADER_C_CALL, ") column is not found in the column name of barcode_airr.tsv file (TRUST4 output).\nCheck the variable in analysisParams.R or you're column in the tsv file."))
    }
    if (!COLUMN_HEADER_V_CALL %in% colnames(file_air_tsv)){
      plate_errors_trust4 <- c(plate_errors_trust4, paste0("<BR>\nERROR : V_CALL (", COLUMN_HEADER_V_CALL, ") column is not found in the column name of barcode_airr.tsv file (TRUST4 output).\nCheck the variable in analysisParams.R or you're column in the tsv file."))
    }
    if (!COLUMN_HEADER_J_CALL %in% colnames(file_air_tsv)){
      plate_errors_trust4 <- c(plate_errors_trust4, paste0("<BR>\nERROR : J_CALL (", COLUMN_HEADER_J_CALL, ") column is not found in the column name of barcode_airr.tsv file (TRUST4 output).\nCheck the variable in analysisParams.R or you're column in the tsv file."))
    }
    if (!COLUMN_HEADER_PRODUCTIVE %in% colnames(file_air_tsv)){
      plate_errors_trust4 <- c(plate_errors_trust4, paste0("<BR>\nERROR : PRODUCTIVE (", COLUMN_HEADER_PRODUCTIVE, ") column is not found in the column name of barcode_airr.tsv file (TRUST4 output).\nCheck the variable in analysisParams.R or you're column in the tsv file."))
    }
    if (!COLUMN_HEADER_CELL_ID %in% colnames(file_air_tsv)){
      plate_errors_trust4 <- c(plate_errors_trust4, paste0("<BR>\nERROR : COLUMN_HEADER_CELL_ID (", COLUMN_HEADER_CELL_ID, ") column is not found in the column name of barcode_airr.tsv file (TRUST4 output).\nCheck the variable in analysisParams.R or you're column in the tsv file."))
    }
  }
  ...
  if (length(plate_errors_trust4) > 0) {
    cat(" <BR>")
    cat("\n••••• ERROR DETECTED IN OUTPUT TRUST4 FROM SCRIPT '01_prepareData' •••••\n")
...
if (length(plate_errors_zUMIs_output) > 0 | length(plate_errors_ERCC) > 0 | length(plate_errors_cell_barcode_well) > 0 | length(plate_errors_indexsort) > 0 | length(plate_errors_metadata) > 0 | length(plate_errors_trust4) > 0){
  stop("Some error has been detecting. Stop analysis. This error comes from script : '01_prepareData' ")
}

# Environment cleanup: removes temporary objects
if( exists( "plate_errors_zUMIs_output")) rm("plate_errors_zUMIs_output")
if( exists( "plate_errors_ERCC")) rm("plate_errors_ERCC")
if( exists( "plate_errors_cell_barcode_well")) rm("plate_errors_cell_barcode_well")
if( exists( "plate_errors_indexsort")) rm("plate_errors_indexsort")
if( exists( "plate_errors_metadata")) rm("plate_errors_metadata")
if( exists( "plate_errors_trust4")) rm("plate_errors_trust4") 
